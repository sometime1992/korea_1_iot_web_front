...
const arrrowMap = numbers.map(value =>  value * value);
...
const arroFilter = basicArray.filter((value) => {
  return value % 2 === 0;
});
...
// >> 문자열데이터.toUpperCase()
const upperCars = cars.map(value => value.toUpperCase());
console.log(upperCars);

// 3. 전체 배열의 요소 중 문자열의 길이가 4보다 큰 요소만 반환 - filter
// >> 문자열데이터.length
const carsFilter = cars.filter(function(value) {
