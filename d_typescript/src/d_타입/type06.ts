...
// >> 해당 값을 자동으로 리터럴 값으로 유추(문자 그대로를 타입으로 인식)
...
function setAlignment(align: 'left' | 'center' | 'right') {
  ...
  // container.style.testAlign = align;
}
...
let user: UserType = {
  name: '이승아',
  // height: 170 '170' 형식은 '169' 형식에 할당할 수 없습니다.ts(2322)
  height: 169
}
...
greet(p1); // Person과 구조가 일치하기 떄문에 Person 취급
greet(p2); // 구조적 타이핑에 의해 Person 취급 (hobby속성을 무시)
// greet(p3); // >> Person 타입 속성 구조와 일치하지 않음(Person 타입에 꼭 필요한 age값이 없음)
...
type UserDataType = {
  ...
  // 키의 타입과 값의 타입을 미리 명시
  [key: string]: string | number | boolean; // 키는 string 사용을 권장(객체는 따옴표 처리하지 않아도 자동으로 문자열로 인식)
