...
type ArroeFuncType = (name: string) => string;

const arrowFunc: ArroeFuncType = (name) => {
  return `Hi ${name}`
}
...
function sum(a: number, b: number, ...c: number[]) {
  return c.reduce((c1, c2) => c1 + c2, 0/*초기값 0*/); // c1 += c2
}
