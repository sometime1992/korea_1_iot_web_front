// export const tmp = ''; === {}
{
  ...
  // 전체 사용자 데이터를 Users의 배열에 관리
  //전체 페이지 - 인터페이스 , 페이지 안의 기능 - 타입
  type Users = User[];
  ...
  const createUserCard = (user: User): HTMLElement => {
    ...
    // class스와 이름이 겹치기 떄문에 html의 clss를 사용할 경우에는 className 사용한다
  ...
  // dom 컨텐츠가 전부 메모리에 올라가면 init을 실행
}
