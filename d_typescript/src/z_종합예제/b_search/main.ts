{
  ...
  // async, await 프로미스 기반 -> 리턴값 : 프로미스 지정
  const fetchUsers = async (): Promise<Users> => {
    try {
      ...
      // response으로 가져오는 데이터들중 특정 데이터가 json형식 이기 때문에 .json()으로 분석해주려고 사용
      return users;
    } catch (error) {
  ...
  const sortUsers = (users: Users, key: "name" | "email"): Users => {
    ...
    // >> 반환값(음수: a가 b보다 앞 / 0: a와 b가 동일 / 양수: b가 a보다 앞)//!모르면 패스
    // >> 오름차순 정렬
