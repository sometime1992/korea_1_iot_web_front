...
function generic<T>(arg: T): T { // 함수 정의 시 타입 변수 지정 // 제네릭은 리턴값이 머가 들어올지 모르기 떄문에 지정을 잘 하지 않는다
  // 타입 변수 T (단일[꺽쇠괄호안 T한개] 타입 변수)
  return arg;
}

let str1 = generic<string>('안녕하세요'); // 함수 호출 시 타입 변수에 활용할 타입을 명시
let str2 = generic('hello'); // 오류 X - 제네릭의 경우 타입 변수에 값을 유추 가능 : 가능한 제네릭 타입을 생략하지 않는다
