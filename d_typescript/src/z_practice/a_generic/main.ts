...
class TaskManager<T> {
  ...
  removeTask(id: number): void {
    // 기존 tasks 배열에서 매개변수로 받은 id와 일치하지 않는
  ...
  renderTasks(containerId: string): void {
    ...
    const container = document.getElementById(containerId) as HTMLUListElement;// HTMLUListElement타입 단언
    ...
    this.tasks.forEach(task => {
      ...
      // A요소.inserBefor(B요소, A요소.firstChilde)
      // : A요소 안에 B요소를 "삽입"
      // >> A요소의 맨 앞의 요소보다 "li.firstChild전에"
